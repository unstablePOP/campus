...
<%-- 아이디 input 부분 --%>
			<form action="/main/joinBusiness.do" method="post" id="joinDo">
                <div id="idInforInputWrap">
                    <h3>사업자 아이디 정보</h3>
                    <dl>
                        <dt class="firstDt"><span class="stars">*</span> 아이디</dt>
                        <dd class="firstDt">
                            <div class="inputTextWrap"><input type="text" name="businessId" class="inputText" placeholder="사용하실 아이디를 입력하신 후 중복확인을 눌러주세요."></div>
                    ...
                    <dl>
                        ...
                        <dd>
                            <div class="inputTextWrap"><input type="password" name="businessPwd" class="inputText" placeholder="(8~15글자)영어 대소문자 + 숫자 + 특수문자로 구성되어야 합니다."></div>
                    ...
                    <dl>
                        ...
                        <dd class="lastDt">
                            <div class="inputTextWrap"><input type="password" name="businessPwdRe" class="inputText" placeholder="비밀번호를 재입력해주세요."></div>
...
<%-- 개인 정보 input 부분 --%>
                <div id="userInforInputWrap">
                    <h3>사업자 정보</h3>
                    <dl>
                        <dt class="firstDt"><span class="stars">*</span> 회사 이름</dt>
                        <dd class="firstDt">
                            <div class="inputTextWrap"><input type="text" name="businessName" class="inputText" placeholder="회사 이름을 입력해주세요."></div>
                            <div class="inputDescription" id="nameDescription">회사 전체 이름을 적어주세요.</div>
                        </dd>
                    </dl>
                    <dl>
                        <dt><span class="stars">*</span>회사 연락처</dt>
                        <dd>
                            <div class="inputTextWrap"><input type="text" name="businessPhone" class="inputText" placeholder="' - ' 없이 입력해주세요."></div>
                            <div class="inputDescription" id="phoneDescription">현재 연락 가능한 번호를 입력해주세요.</div>
                        </dd>
                    </dl>
                    <dl>
                        <dt><span class="stars">*</span> 회사 주소</dt>
                        <dd>
                            <div class="inputTextWrap"><input type="text" name="businessAddress" id="businessAddress" class="inputText"></div>
                            <div class="inputBtnWrap"><input type="button" onclick="addrSearch()" class="inputBtn" value="주소 찾기"></div><br>
                            <div class="inputDescription" id="addrDescription">회사 주소를 입력해주세요.</div>
                        </dd>
                    </dl>
                    <dl>
                        <dt><span class="stars">*</span> 회사 URL</dt>
                        <dd>
                            <div class="inputTextWrap"><input type="text" name="businessUrl" id="businessUrl" class="inputText"></div>
                            <div class="inputDescription" id="urlDescription">회사 URL을 입력해주세요.</div>
                        </dd>
                    </dl>
                    <dl>
                        <dt class="lastDt"><span class="stars">*</span>회사 이메일</dt>
                        <dd class="lastDt">
                            <div class="inputTextWrap"><input type="email" class="inputText" disabled=ture value="${param.email }"></div>
	                        <input type="hidden" value="${param.email }" name="businessEmail">
...
<%-- 가입하기 button 부분 --%>
    ...
    <script>
<%-- ID 유효성 검사 --%>
    	$("input[name='businessId']").keyup(function(){
...
<%-- Pwd 유효성 검사 --%>
    	$("input[name='businessPwd']").keyup(function(){
...
<%-- Pwd 재검사 --%>
    	$("input[name='businessPwdRe']").keyup(function(){
    		if($(this).val()==$("input[name='businessPwd']").val()){
...
<%-- 이름 유효성 검사 --%>
    	$("input[name='businessName']").keyup(function(){
    		if($(this).val()!=""){
...
<%-- 연락처 유효성 검사 --%>
    	$("input[name='businessPhone']").keyup(function(){
...
<%-- 회사 URL 입력 여부 검사 --%>
    	$("#businessUrl").focus(function(){
    		if($(this).val()!==""){
    			$('#urlDescription').css('color','dodgerblue');
    		}else{
    			$('#urlDescription').css('color','red');
    		}
    	});
    	$("#businessUrl").blur(function(){
    		if($(this).val()!==""){
    			$('#urlDescription').css('color','dodgerblue');
    		}else{
    			$('#urlDescription').css('color','red');
...
<%-- 전체 항목 검사 --%>
    	function joinCheck(){
    		...
    		var url = $("#urlDescription").css('color')=="rgb(30, 144, 255)";
    		if(id&&pwd&&pwdRe&&name&&phone&&addr&&url){
    			$('#joinDo').submit();
    		}else{
    ...
    function addrSearch() {
        new daum.Postcode({
            oncomplete: function(data) {
                ...
                document.getElementById("businessAddress").value = roadAddr;
                // 커서 포커스
